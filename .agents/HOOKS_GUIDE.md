...
| :--- | :--- | :--- | :--- | :--- |
| **1. خطاف البدء والافتتاح** | `[prompt-engineer]`, `[agent-optimizer]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"بسم الله"`, `"بسم الله الرحمن الرحيم"` | تهيئة الجلسة، التحقق من القواعد الفنية واللغوية، وترشيد الاستهلاك. |
| **2. خطاف هندسة السياق** | `[prompt-engineer]`, `[agent-optimizer]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"سياق المشروع"`, `"هندسة السياق"` | إنشاء وتحديث ملف `PROJECT_CONTEXT.md` لضمان التوجيه وتفادي الانحراف التقني. |
| **3. خطاف الإشراف العام وتصحيح المسار** | `[prompt-engineer]`, `[agent-optimizer]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro (Thinking)` | `"تصحيح مسار"`, `"تعديل البرومبت"`, `"خطأ بالخطاف"`, `"تصحيح خطاف"` | تصحيح الخطافات الخاطئة والبرومبتات غير الاحترافية وتوجيهها للمسار الصحيح تلقائياً. |
| **4. خطاف خط الإنتاج البرمجي** | `[code-architect]`, `[android-kotlin-pro]`, `[jetpack-compose-ui]`, `[code-reviewer-quality]`, `[clean-code-guard]`, `[android-testing]`, `[test-guard]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro Experimental` | `"ابدأ ميزة"`, `"تطوير ميزة"`, `"اصنع ميزة"`, `"إصلاح ميزة"` | تنفيذ دورة التطوير الكاملة (المعمارية، البرمجة، حراسة الكود النظيف، والفحص والاختبار). |
| **5. خطاف التكامل الفني** | `[devops-deployer]`, `[backend-architect]` | الأساسي: `Claude 3.5 Sonnet` <br> البديل: `Gemini 2.0 Pro` | `"ربط API"`, `"تكامل خارجي"` | ربط وتكامل الخدمات الخارجية بعد مراجعة وثائقها الرسمية وتأمين المتغيرات. |
| **6. خطاف تدقيق الصيانة** | `[code-reviewer-quality]`, `[clean-code-guard]`, `[performance-optimizer]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Flash` | `"تدقيق صيانة"`, `"مراجعة DRY"`, `"كود نظيف"` | مراجعة الكود البرمجي بانتظام لمنع التكرار وتراكم الديون التقنية وتطبيق معايير الكود النظيف. |
| **7. خطاف الفحص الأمني** | `[security-auditor]`, `[code-reviewer-quality]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"فحص أمني"`, `"مراجعة أمان"` | مراجعة ثغرات الأمان والتحقق من صحة المدخلات وتشفير البيانات وتأمين الوصول. |
| **8. خطاف مراجعة وتحديث المكتبات** | `[github-talent-scout]`, `[code-architect]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"تحديث الاعتماديات"`, `"تحديث المكتبات"` | فحص ملفات Gradle وتحديث المكتبات والاعتماديات القديمة وتجنب التعارضات. |
| **9. خطاف الفحص والاختبار التلقائي** | `[android-testing]`, `[test-guard]`, `[code-reviewer-quality]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"قم بالاختبار"`, `"جاهز للتجربة"` | تشغيل اختبارات الوحدة وفحص جودة الكود واختبار السلوك الفعلي بدون Mock مفرط. |
| **10. خطاف معالجة الأخطاء والأعطال** | `[debugger]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"حدث خطأ"`, `"التطبيق توقف"`, `"Crash"` | تحليل سجلات الأخطاء والـ Stack Trace وعزل الكود المسبب للمشكلة وإصلاحه. |
| **11. خطاف إدارة النسخ والمستودعات** | `[git-github-manager]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"نظم جيت"`, `"ارفع لجيتهاب"`, `"تحديث المستودع"`, `"git commit"` | تهيئة المشاريع الجديدة على Git، إدارة التفرعات، وحل تعارضات الدمج ورفعها لـ GitHub بشكل نظيف. |
| **12. خطاف تحديث الإنجازات** | `[git-github-manager]`, `[docs-guard]` | الأساسي: `Claude 3.5 Sonnet` <br> البديل: `Gemini 2.0 Flash` | `"حدث الإنجازات"`, `"تحديث README"`, `"وثق الإنجازات"` | صياغة التعديلات كإنجازات ومطابقتها برمجياً عبر docs-guard ثم كتابتها في README.md ورفعها لـ GitHub. |
| **13. خطاف الذاكرة المستدامة** | `[persistent-memory-engine]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro (Thinking)` | `"حفظ ذاكرة"`, `"تحديث السياق"`, `"سجّل هذا"` | التقاط الدروس والقرارات وتخزينها في MEMORY_STORE.md وتصديرها للوكلاء. |
| **14. خطاف النجاح والتحليل البعدي** | `[agent-optimizer]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"تم بنجاح"`, `"انتهى المشروع بنجاح"` | إجراء تحليل بعدي للمشروع وتحديث سجل التعلم وسد الثغرات البرمجية. |
| **15. خطاف تنظيم وترتيب الأجهزة الشامل** | `[windows-c-drive-optimizer]`, `[windows-file-organizer]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"رتب جهازى"`, `"فرز الملفات"`, `"ترتيب ملفات"` | ترتيب ملفات جذور الأقراص بالكامل مع تنظيف وتحسين مساحة قرص الـ C بآن واحد. |
| **16. خطاف ترتيب الملفات لمسار محدد** | `[windows-file-organizer]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"رتب المسار"`, `"نظم المجلد"`, `"ترتيب مسار"` | تنظيم وفرز الملفات داخل مجلد أو مسار مخصص يحدده المستخدم بذكاء ودقة. |
| **17. خطاف إدارة تليجرام** | `[devops-deployer]`, `[persistent-memory-engine]` | الأساسي: `Gemini 2.0 Flash` <br> البديل: `Claude 3.5 Haiku` | `"شغل البوت"`, `"تفعيل تليجرام"`, `"Telegram Bot"` | تشغيل بوت تليجرام في الخلفية بنمط الاستماع للطلبات عن بعد دون تعارض. |
| **18. خطاف التدقيق البرمجي الشامل وحراسة الجودة** | `[code-reviewer-quality]`, `[clean-code-guard]`, `[test-guard]`, `[docs-guard]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro (Thinking)` | `"تدقيق الجودة"`, `"فحص الكود النظيف"`, `"حراسة الجودة"`, `"clean code audit"` | تفعيل بوابات الحماية الثلاث لفحص الكود النظيف، سلامة الاختبارات، ومطابقة التوثيق. |
| **19. خطاف استكشاف وتكامل الموارد العالمية** | `[resource-scout-integrator]`, `[github-talent-scout]`, `[skill-forge-builder]`, `[agent-optimizer]`, `[frontend-design-builder]`, `[git-github-manager]` | الأساسي: `Claude 3.7 Sonnet (Thinking)` <br> البديل: `Gemini 2.0 Pro` | `"استكشف المورد"`, `"حلل المستودع"`, `"تكامل المورد"`, `"scout resource"`, `"integrate repo"` | تحليل المستودعات الخارجية واستيراد المهارات وتعميمها وتحديث الخطافات والإكسيل وتطبيق مكتبة الأوامر ومزامنة المنظومة. |
