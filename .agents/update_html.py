...
md_path = r'F:\AI PROJECTS\Blind App\.agents\HOOKS_GUIDE.md'
html_path = r'F:\AI PROJECTS\Quran_Records\with antigravity\New Crew\03_Dynamic_Prompt_Library\index.html'
...
# Fix the invalid JSON generated by re.sub earlier
# First, let's extract the start index
...
if start_idx != -1:
    ...
    if end_idx != -1:
        # We replace from start_idx up to end_idx + len(end_marker)
        # We will inject the new json and activeTab properly
        end_after_marker = end_idx + len(end_marker)
        new_html = html_content[:start_idx] + f"let libraryData = {json_str};\n        let activeTab" + html_content[end_after_marker:]
