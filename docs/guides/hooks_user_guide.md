...
مرحباً بك في غرفة التحكم. هذا الدليل يوضح لك **الترتيب الزمني الصارم** لاستخدام الخطافات التلقائية (من الخطاف الأول وحتى الخطاف رقم 20) لإدارة دورة حياة تطوير أي ميزة أو مشروع بنجاح من بداية الجلسة وحتى تسليمها وتوثيقها ومزامنتها سحابياً.
...
flowchart TD
    ...
    subgraph Phase4 ["📦 المرحلة الرابعة: التوثيق، الذاكرة، والمزامنة السحابية"]
        ...
        H14 --> H20["20. المزامنة السحابية الشاملة (مزامنة المنظومة)"]
    end
...
4. **الخطاف #8 (خطاف مراجعة وتحديث المكتبات والنماذج الذكية)**
   ...
   - **النموذج المقترح:** `Gemini 3.7 Flash (High)`
   - **الهدف:** تحديث إصدارات Gradle والمكتبات، وفحص بوابات الإنترنت الرسمية (`antigravity.google` و `google.dev`) لتحديث مصفوفة النماذج الذكية ودليل hooks_user_guide.md آلياً دون لقطات شاشة.
...
### 📦 المرحلة الرابعة: التوثيق، الذاكرة، والمزامنة السحابية (Wrap-up & Full Sync)
*تُستخدم لإنهاء الجلسة وحفظ الذاكرة المعرفية والمزامنة الشاملة مع GitHub.*
...
12. **الخطاف #12 (خطاف تحديث الإنجازات والتوثيق)**
    - **كلمة التفعيل:** `حدث الإنجازات` أو `تحديث README`
    - **النموذج المقترح:** `Gemini 3.7 Flash (High)`
    - **الهدف:** صياغة التعديلات كإنجازات واضحة، وتحديث ملف `README.md` وملف `hooks_user_guide.md` والرفع التلقائي.
...
15. **الخطاف #20 (خطاف المزامنة السحابية الشاملة ومطابقة المنظومة) - *خاتمة مسار الإنتاج***
    - **كلمة التفعيل:** `مزامنة المنظومة` أو `مزامنة المستودع العام` أو `sync ecosystem`
    - **النموذج المقترح:** `Gemini 3.7 Flash (High)` / البديل: `Claude Sonnet 4.6 (Thinking)`
    - **الهدف:** مطابقة ومزامنة كافة الإضافات (Plugins)، المراجع (References)، إعدادات MCP، المهارات والوكلاء، وتحديث ملفات الإكسيل وتطبيق مكتبة الأوامر HTML، ودفع التحديثات بالكامل إلى مستودع [Claude-Antigravity-Workspace](https://github.com/ibrahimalkateb965-tech/Claude-Antigravity-Workspace) بنقرة واحدة بتطابق 100%.
