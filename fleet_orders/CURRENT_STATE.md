...
- **Next orders — the `:app` cut-overs, one at a time, each with the full regression gate**
  ...
  6. ~~**ORDER-P1-006-B** wire `QuranViewModel` onto `AudioEngine`~~ — **DONE 09:35, see §5.** Ibrahim chose
     **(a)**. Written and executed directly by Claude Code CLI (not dispatched — see the report's §0: no
     `@Composable`, no Gradle/CI/DB content, a precise behaviour-preserving refactor of one ~750-line file
     where a wrong edit is a silent runtime regression, not a compile error). `QuranViewModel` now holds an
     `AudioEngine` (wrapping the same `MediaController`) instead of driving `Player` directly; every
     `Player.Listener` branch became an `AudioEngineEvent` branch with an identical body.
     `QuranAudioService` untouched. +124/−145, one file. R8 now genuinely retains the audio package
     (`AudioEngine -> ha`, `AudioEngineEvent -> ma` in `mapping.txt`, 327 references — verified, since the
     release APK stayed byte-identical to P0-009/P0-010/P1-006 for a 4th time, which needed checking rather
     than trusting; explained by APK signing-block padding, not a caching bug).
  7. **Nothing is queued.** **Next is Ibrahim's call:**
...
| P1-006-B — wire `QuranViewModel` onto `AudioEngine` | ✅ **QUALITY GATE: PASS** 2026-09-12 09:35 (session 5). Written and executed directly by Claude Code CLI (no `@Composable`, no Gradle/CI/DB — not OpenCode's or Antigravity's reserved territory; a precise behaviour-preserving refactor, see the report's §0 for the rationale). `QuranViewModel.kt` +124/−145, the only file touched: `mediaController`-direct calls and the anonymous `Player.Listener` replaced by `audioEngine: AudioEngine?` + collecting `engine.events` into a `handleAudioEngineEvent` function with one branch per `AudioEngineEvent` subtype, each an identical-body copy of the old listener override. Media ids now built via `AyahTrackId.encode` (byte-identical string), network-error classification moved into `AudioEngine.android.kt` (added by P1-006), dead `isNetworkRelatedError` deleted. `QuranAudioService` untouched. Gate: `:app` 35/35, `assembleDebug`, `assembleRelease` (one daemon OOM retry — proactively killed an idle Kotlin compile daemon first this time), `:shared` 32/32 (UP-TO-DATE, unchanged). R8 verification: release APK stayed byte-identical to P0-009/P0-010/P1-006 (5,867,725 B) for a 4th time even though the audio package is now genuinely used — confirmed via `mapping.txt` (327 `com.aistudio.quranblind.audio` references, real classes retained/renamed, not `R8$$REMOVED$$CLASS`) rather than trusting the coincidental byte count; explained by APK signing-block/zip-alignment padding. Report: `reports/ORDER_P1_006B_REPORT_COMMANDER.md`. |
